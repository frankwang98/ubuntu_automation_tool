...
class UbuntuAutomationTool:
    ...
    def update_script(self) -> None:
        ...
        try:
            # 临时克隆仓库到/tmp目录
            self.run_command("rm -rf /tmp/ubuntu_automation_tool")
            self.run_command("git clone https://github.com/frankwang98/ubuntu_automation_tool.git /tmp/ubuntu_automation_tool")
            
            # 检查是否有新版本
            with open("/tmp/ubuntu_automation_tool/ubuntu_tool.py", "r") as f:
                for line in f:
            ...
            if new_version != VERSION.split(" - ")[0]:
                ...
                if confirm == 'y':
                    ...
                    # 替换脚本
                    self.run_command(f"cp /tmp/ubuntu_automation_tool/ubuntu_tool.py {script_path}")
